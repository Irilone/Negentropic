    ...
    try {
      ...
      return projects.map(project => {
        ...
        const sortedImages = project.project_images
          ...
          .map((img: any) => `${storageUrl}/public/entropy-assets/${project.slug}/${img.url}`);

        // Always use the project card image for the cover
        const coverImagePath = `${storageUrl}/public/entropy-assets/${project.slug}/${project.slug}-projectcard-img.png`;
